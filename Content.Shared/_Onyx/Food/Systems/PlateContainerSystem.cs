...
public sealed class PlateContainerSystem : EntitySystem
{
    ...
    private void OnInteractUsing(Entity<PlateContainerComponent> ent, ref AfterInteractUsingEvent args)
    {
        ...
        {
            _popup.PopupClient(Loc.GetString(GetPopup(result)), ent.Owner, args.User);
            args.Handled = true;
            return;
        }
    ...
    private bool PassesFilters(PlateContainerComponent component, EntityUid item)
    {
        ...
        return prototype != null && component.WhitelistPrototypes.Contains(prototype) ||
               component.WhitelistTags.Count > 0 && _tag.HasAnyTag(item, component.WhitelistTags);
    }
