/*
 * This file is sublicensed under MIT License
 * https://github.com/space-wizards/space-station-14/blob/master/LICENSE.TXT
 */

using Content.Shared._Onyx.ZLevels.Core.Components;
using Content.Shared._Onyx.ZLevels.Core.EntitySystems;
using Content.Shared.Light.Components;
using Content.Shared.Light.EntitySystems;
using Content.Shared.Maps;
using Robust.Shared.Map;
using Robust.Shared.Map.Components;

namespace Content.Shared._Onyx.ZLevels.Roof;

/// <summary>
/// Systems that automatically covers tiles with roofs (or removes roofs)
/// if there is a tile on one of the levels above in the ZLevels network.
/// </summary>
public abstract class CESharedRoofSystem : EntitySystem
{
    [Dependency] protected readonly CESharedZLevelsSystem ZLevel = default!;
    [Dependency] protected readonly SharedRoofSystem Roof = default!;
    [Dependency] protected readonly SharedMapSystem Map = default!;
    [Dependency] protected readonly ITileDefinitionManager TilDefMan = default!;

    protected EntityQuery<MapGridComponent> GridQuery;
    protected EntityQuery<RoofComponent> RoofQuery;

    private readonly Dictionary<Vector2i, bool> _roofMapBuffer = new();
    private readonly List<Vector2i> _roofIndicesBuffer = new();
    private readonly List<Vector2i> _activeRoofIndicesBuffer = new();
    private readonly List<Vector2i> _nextActiveRoofIndicesBuffer = new();

    public override void Initialize()
    {
        base.Initialize();

        GridQuery = GetEntityQuery<MapGridComponent>();
        RoofQuery = GetEntityQuery<RoofComponent>();

        SubscribeLocalEvent<CEZLevelMapComponent, TileChangedEvent>(OnTileChanged);
    }

    /// <summary>
    /// When changing tiles, we iteratively go down to the end of the ZLevels network, repeatedly calculating whether the tiles at the bottom now have a roof or not.
    /// </summary>
    private void OnTileChanged(Entity<CEZLevelMapComponent> ent, ref TileChangedEvent args)
    {
        if (!GridQuery.TryComp(ent, out var currentMapGrid))
            return;
        if (!RoofQuery.TryComp(ent, out var currentRoof))
            return;

        if (args.Changes.Length == 0)
            return;

        _roofMapBuffer.Clear();
        var roofMap = _roofMapBuffer;
        _roofIndicesBuffer.Clear();
        var roofIndices = _roofIndicesBuffer;
        foreach (var change in args.Changes)
        {
            var tileDef = (ContentTileDefinition)TilDefMan[change.NewTile.TypeId];

            var roovedAbove = Roof.IsRooved((ent, currentMapGrid, currentRoof), change.GridIndices);
            var roovedTile = !tileDef.Transparent;
            roofMap[change.GridIndices] = roovedAbove || roovedTile;
        }

        var mapsBelow = ZLevel.GetAllMapsBelow(ent);

        if (mapsBelow.Count == 0)
            return;

        roofIndices.AddRange(roofMap.Keys);
        _activeRoofIndicesBuffer.Clear();
        _activeRoofIndicesBuffer.AddRange(roofIndices);

        foreach (var mapBelow in mapsBelow)
        {
            if (_activeRoofIndicesBuffer.Count == 0)
                break;

            if (!GridQuery.TryComp(mapBelow, out var mapGridBelow))
                continue;

            var roofBelow = EnsureComp<RoofComponent>(mapBelow);
            _nextActiveRoofIndicesBuffer.Clear();

            foreach (var indices in _activeRoofIndicesBuffer)
            {
                var rooved = roofMap[indices];
                Roof.SetRoof((mapBelow, mapGridBelow, roofBelow), indices, rooved);

                var hasTile = Map.TryGetTile(mapGridBelow, indices, out var tile) && !tile.IsEmpty;
                if (hasTile)
                {
                    // Any solid tile on this level becomes a stable "roof source" for all deeper maps.
                    if (!rooved)
                        roofMap[indices] = true;
                    continue;
                }

                _nextActiveRoofIndicesBuffer.Add(indices);
            }

            _activeRoofIndicesBuffer.Clear();
            _activeRoofIndicesBuffer.AddRange(_nextActiveRoofIndicesBuffer);
        }
    }
}
