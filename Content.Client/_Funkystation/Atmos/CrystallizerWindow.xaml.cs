    ...
    public sealed partial class CrystallizerWindow : FancyWindow
    {
        ...
        private void InitializeRecipeButtons()
        {
            ...
            foreach (var recipe in _prototypeManager.EnumeratePrototypes<CrystallizerRecipePrototype>())
            {
                ...
                {
                    Text = Loc.GetString(recipe.Name),
        ...
        private void SelectRecipeButton(Button button, CrystallizerRecipePrototype? recipe = null)
        {
            ...
            if (recipe == null)
            {
                var selectMessage = new Label { Text = Loc.GetString("crystallizer-ui-select-recipe-prompt") };
            ...
            label = new Label { Text = Loc.GetString("crystallizer-ui-recipe-minimum", ("temp", recipe.MinimumTemperature)) };
            RequirementsContainer.AddChild(label);
            label = new Label { Text = Loc.GetString("crystallizer-ui-recipe-maximum", ("temp", recipe.MaximumTemperature)) };
            RequirementsContainer.AddChild(label);
            ...
            for (int i = 0; i < recipe.MinimumRequirements.Length && i < Atmospherics.TotalNumberOfGases; i++)
            {
                ...
                {
                    var gas = (Gas) i;
                    var gasName = Loc.GetString(_atmosphere.GetGas(gas).Name);
                    var gasLabel = new Label { Text = Loc.GetString("crystallizer-ui-gas-requirement", ("gas", gasName), ("moles", recipe.MinimumRequirements[i])) };
            ...
            RequirementsContainer.AddChild(label);
            label = new Label { Text = recipe.EnergyRelease > 0
                ? Loc.GetString("crystallizer-ui-reaction-exothermic")
                : Loc.GetString("crystallizer-ui-reaction-endothermic") };
            RequirementsContainer.AddChild(label);
            ...
            for (int i = 0; i < recipe.MinimumRequirements.Length && i < Atmospherics.TotalNumberOfGases; i++)
            {
                if (recipe.MinimumRequirements[i] > 0)
                {
                    ...
                    var molesLabel = new Label { Text = Loc.GetString("crystallizer-ui-moles", ("moles", $"{moles:F2}")), Margin = new Thickness(5) };
