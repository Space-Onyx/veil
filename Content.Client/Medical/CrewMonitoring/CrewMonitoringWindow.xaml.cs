...
public sealed partial class CrewMonitoringWindow : FancyWindow
{
    ...
    private int _monitorFloor;
    private EntityUid? _monitorMapUid;
    ...
    public void Set(string stationName, EntityUid? mapUid)
    {
        ...
        _monitorMapUid = ZLevelFloorSelectorHelper.ResolveMapUid(_entManager, mapUid);

        if (_monitorMapUid != null)
        ...
        EntityCoordinates? monitorCoords)
    {
        ...
        var selectedMapUid = ZLevelFloorSelectorHelper.ResolveSelectedMapUid(_entManager, selectedFloorMap);
        var navMapUid = selectedMapUid ?? _monitorMapUid;
        NavMap.Visible = navMapUid != null;

        if (NavMap.MapUid != navMapUid)
        {
        ...
        if (_trackedEntity != null && uniqueSensors.All(sensor => sensor.SuitSensorUid != _trackedEntity.Value))
            _trackedEntity = null;
    ...
    private void UpdateFloors(List<int> floors, int selectedFloor, int monitorFloor)
    {
        ...
        {
            FloorSelector.AddItem(ZLevelFloorSelectorHelper.FormatRelativeFloor(floors[i], _monitorFloor), i);
        }
