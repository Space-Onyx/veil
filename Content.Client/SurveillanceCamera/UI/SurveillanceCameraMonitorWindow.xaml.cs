...
public sealed partial class SurveillanceCameraMonitorWindow : FancyWindow // Goobstation
{
        ...
        EntityCoordinates? monitorCoords) // Goobstation
    {
        ...
        EntityUid? navMapUid = ZLevelFloorSelectorHelper.ResolveSelectedMapUid(_entManager, selectedFloorMap);

        foreach (var (camera, (name, (ent, coordinates))) in cameras)
        {
            navMapUid ??= ZLevelFloorSelectorHelper.ResolveMapUidFromEntityOrCoordinates(_entManager, ent, coordinates);
        ...
        var resolvedMapUid = navMapUid ?? _monitorMapUid;
        NavMap.Visible = resolvedMapUid != null;

        if (NavMap.MapUid != resolvedMapUid)
        {
            NavMap.MapUid = resolvedMapUid;
