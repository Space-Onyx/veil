...
public sealed class DepartmentBanCommand : IConsoleCommand
{
    ...
    public async void Execute(IConsoleShell shell, string argStr, string[] args)
    {
        ...
        var roles = new List<string>(); // Onyx - Ban Webhook
        foreach (var job in departmentProto.Roles)
        {
            ...
            roles.Add(job.Id); // Onyx - Ban Webhook
        }
        _banManager.WebhookUpdateRoleBans(targetUid, located.Username, shell.Player?.UserId, null, targetHWid, roles, minutes, severity, reason, now);  // Onyx - Ban Webhook
    }
