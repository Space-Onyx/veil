    ...
    public void AccUpdate()
    {
        ...
        while (query.MoveNext(out var uid, out var comp))
        {
            ...
            else
            {
                ...
                {
                    // If this while loop causes problems, blame whoever put 1.78e308 bananium in the centrifuge
                    while (comp.ExtractedFuel > 1) 
                    {
                        var bananiumStack = Spawn("MaterialBananium1", Transform(uid).Coordinates);
                        _stackSystem.SetCount(bananiumStack, Math.Clamp((int)Math.Floor(comp.ExtractedFuel), 1, _stackSize));
                        comp.ExtractedFuel -= _stackSystem.GetCount(bananiumStack);
                        _stackSystem.TryMergeToContacts(bananiumStack);
                    }
