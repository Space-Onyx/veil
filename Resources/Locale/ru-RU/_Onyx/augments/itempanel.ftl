# ItemPanel Action
ent-ActionAugmentToggleItemPanel = Переключить панель предмета
    .desc = Извлечь или втянуть встроенный предмет

ent-BaseAugmentItemPanel = { ent-BaseArmAugment }
    .desc = { ent-BaseArmAugment.desc }

ent-AugmentManitBladePanel = имплант клинка богомола
    .desc = Имплант руки, содержащий клинок богомола. Совместим с обеими руками.
ent-AugmentStunnerPanel = имплант панели станнера
    .desc = Имплант руки со встроенным станнером. Совместим с обеими руками.
ent-AugmentStunbatonPanel = имплант панели дубинки-шокера
    .desc = Имплант руки со встроенной дубинкой-шокером. Совместим с обеими руками.
ent-AugmentLaserPistolPanel = имплант панели лазерного пистолета
    .desc = Имплант руки со встроенным самозаряжаемым лазерным пистолетом. Совместим с обеими руками.

# ItemPanel Examine Text
augment-examine-item-panel = Панель предмета
augment-examine-mantis-blade-panel = Панель Клинка богомола
augment-examine-stunner-panel = Панель станнера
augment-examine-stunbaton-panel = Панель дубинки-шокера
augment-examine-laser-pistol-panel = Панель лазерного пистолета

# ItemPanel Messages
augment-item-panel-no-hand = У вас нет руки!
augment-item-panel-hand-full = В вашей руке что-то есть!
augment-item-panel-cannot-equip = Ваша рука не способна это удержать!
augment-item-panel-deployed = Из вашей руки появляется { CAPITALIZE($item) }.
augment-item-panel-retracted = { $item } втягивается обратно в вашу руку.
ent-WeaponDisablerAugmentSelfCharging = { ent-WeaponDisabler }
    .desc = { ent-WeaponDisabler.desc }
ent-StunbatonAugmentSelfCharging = { ent-Stunbaton }
    .desc = { ent-Stunbaton.desc }
ent-ActionAugmentToggleItemPanelLeft = Переключить панель предмета (левая рука)
    .desc = Извлечь или втянуть встроенный предмет из левой руки.
ent-ActionAugmentToggleItemPanelRight = Переключить панель предмета (правая рука)
    .desc = Извлечь или втянуть встроенный предмет из правой руки.
